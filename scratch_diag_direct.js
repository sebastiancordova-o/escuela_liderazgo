
const { google } = require('googleapis');

const auth = new google.auth.JWT({
  email: 'plataforma-liderazgo@cumbre-liderazgo-506117.iam.gserviceaccount.com',
  key: '-----BEGIN PRIVATE KEY-----\\nEfNPdVjHNK86Fmu2JwdBcBM0\n-----END PRIVATE KEY-----',
  scopes: ['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive'],
});

const sheets = google.sheets({ version: 'v4', auth });

async function main() {
  try {
    console.log('\n1. Consultando archivo en Google Sheets...');
    const meta = await sheets.spreadsheets.get({
      spreadsheetId: '1P2zzTquHp7Ypu2SJ6el24jEFXAiFWR0o',
    });
    console.log('✅ EXITO: Conexión establecida con Google Sheets!');
    console.log('Título:', meta.data.properties.title);
    console.log('Hojas:');
    meta.data.sheets.forEach(s => console.log(' - ' + s.properties.title));
  } catch (err) {
    console.log('\n❌ RESPUESTA DE GOOGLE API:');
    console.log('Status HTTP:', err.status || (err.response && err.response.status));
    console.log('Mensaje:', err.message);
    if (err.errors) {
      console.log('Detalles de Google:', JSON.stringify(err.errors, null, 2));
    }
  }
}

main();
